...
@Composable
fun RewardCard(
    onNavigate: (title: String, cost: Int, points: Int) -> Unit,
    reward:     RewardItem,
    userPoints: Int
) {
    ...
    ElevatedCard(
        modifier = Modifier
            ...
            .clickable(enabled = isRedeemable) {
                onNavigate(reward.title, reward.pointsRequired, userPoints)
            },
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(text = reward.title,             style = MaterialTheme.typography.titleMedium, color = textColor)
                Text(text = "${reward.pointsRequired} puntos", style = MaterialTheme.typography.bodyMedium,  color = subTextColor)
            }
            if (!isRedeemable) {
                Text(
